...
//    }
    ...
    /*检查数据是否有这个变量是否提交*/
        ...
        if($username==true){
            $id=input('post.id');      //用户名
            $pwd=input('post.pwd');      //密码
        ...
        if(!preg_match("/^[A-Za-z0-9]+$/",$id,$matches)){
            $returnJson=[
                'code'=>10002,
                'msg'=>config('LoginMsg')['testing_error'],
                'data'=>[]
            ];
            return $returnJson;
        }
    ...
    /*验证码校验*/
       if(!captcha_check($code)){
           $returnJson=[
               'code'=>10004,
               'msg'=>config('LoginMsg')['captha_error'],
               'data'=>[]
           ];
           return $returnJson;
       }
       ...
       if(!empty($userInfo)){
           foreach ($userInfo as $key=>$val){
               if($val['ustate']=='锁定'){
                   ...
                   Session::set('uname',$val['uname']);             /*用户名*/
                   Session::set('nickname',$val['nickname']);     /*用户昵称*/
                   Session::set('usid',$val['usid']);               //用户id
                   $returnJson=[
                       'code'=>10000,
